...
void moveEvenItemsToBack(LinkedList *ll)
{
	...
	ListNode *even_ptr = ll->head;
	// 처음 짝수 찾기
	while (even_ptr->item % 2 == 1 && even_ptr != NULL)
	{
		last_odd = even_ptr;
	...
	while (even_ptr != NULL)
	{
		ListNode *next_ptr = even_ptr->next;
		if (next_ptr == NULL)
			break;
		// 다음이 홀수면 last_odd에 붙임
		if (next_ptr->item % 2 == 1)
		{
			if (last_odd == NULL)
			{
				even_ptr->next = next_ptr->next;
				next_ptr->next = ll->head;
				ll->head = next_ptr;
				last_odd = next_ptr;
			}
			else
			{
				even_ptr->next = next_ptr->next;
				next_ptr->next = last_odd->next;
				last_odd->next = next_ptr;
				last_odd = next_ptr;
			}
		}
		// 다음이 짝수면 다음으로 이동
